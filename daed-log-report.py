...
Parse daed logs (logfmt-like key=value) and generate:
  - report.md   (Markdown report)
  - report.html (Self-contained HTML report)

Key features:
  - Supports timezone conversion:
      * --log-tz:      timezone of the daed "time=" field (default: UTC)
      * --display-tz:  timezone used in the report output (default: Asia/Shanghai)
  - Adds timezone info to report overview.
  - Filters internal warnings (non-connection logs) into a separate section.
  - Handles "dialer" values with quotes, e.g. dialer="日本家宽"
  - Handles IPv6 endpoints in msg like "[v6]:port <-> [v6]:443"

Usage:
  python3 daed-log-report.py /path/to/daed.log
  cat /path/to/daed.log | python3 daed-log-report.py

Options:
  -o, --outdir DIR     Output directory (default: current directory)
  --title TEXT         Report title (default: "Daed Network Traffic Analysis Report")
  --no-html            Do not generate HTML
  --no-md              Do not generate Markdown
  --top N              Top N domains in ranking (default: 15)
  --log-tz TZ          Timezone of the log 'time=' field (default: UTC)
  --display-tz TZ      Timezone to display in the report (default: Asia/Shanghai)
"""
...
# --- Parsing: logfmt-ish key=value where values may be quoted ---
...
def split_host_port(endpoint: str) -> Tuple[str, str]:
    ...
    if ep.startswith("[") and "]" in ep:
        ...
        if rest.startswith(":"):
            return host, rest[1:]
        return host, ""
    # Non-bracket: split by last colon
    if ":" in ep:
...
def parse_daed_time(t: str, default_year: int, log_tz: ZoneInfo) -> Optional[_dt.datetime]:
    """
    Parse daed time field like: 'Jan 21 15:55:09' (no year, no tz)
    We interpret it as timezone = log_tz, and return an aware datetime.
    """
    if not t:
        return None
    try:
...
# --- Classification helpers (lightweight and editable) ---
...
def fields_to_conn(fields: Dict[str, str], raw_line: str, log_tz: ZoneInfo) -> Optional[ConnRecord]:
    ...
    # Use current year by default; if you need strict year handling across New Year,
...
def render_markdown(
    ...
    # Time range (convert to display_tz for presentation)
    ...
    for c in sorted(conns, key=sort_key):
        time_short = fmt_time_hms(c.time_dt, display_tz, c.time_raw)
        lines.append(f"### {md_escape(time_short)} | {md_escape(c.app)}")
        lines.append(f"- 源：{md_escape(c.src_ep)}")
        if c.sniffed:
            port = c.dst_port or "443"
            lines.append(f"- 目标：{md_escape(c.sniffed)}:{md_escape(port)}")
        else:
            lines.append(f"- 目标：{md_escape(c.dst_ep)}")
        if c.ip:
            lines.append(f"- 实际对端：{md_escape(c.ip)}")
        lines.append(f"- 网络：{md_escape(c.network or 'unknown')}")
        lines.append(f"- 出站：{md_escape(c.route_label)}")
        lines.append(f"- 拨号器：{md_escape(c.dialer or 'unknown')}")
        lines.append(f"- 策略：{md_escape(c.policy_label)}")
        if c.mac:
            lines.append(f"- MAC：{md_escape(c.mac)}")
        if c.dscp != "":
            lines.append(f"- DSCP：{md_escape(c.dscp)}")
        if c.pid and c.pid != "0":
            lines.append(f"- PID：{md_escape(c.pid)}")
        if c.pname:
            lines.append(f"- 进程：{md_escape(c.pname)}")

        # One-line conclusion
        if c.route_label == "direct":
            verdict = "结论：直连（未经过代理）。"
        elif c.route_label == "proxy":
            verdict = "结论：经代理出站。"
        else:
            verdict = f"结论：使用专用/自定义出站（{md_escape(c.route_label)}）。"
        if c.policy_label == "fixed":
            verdict += " 命中固定规则。"
        elif c.policy_label and c.policy_label != "unknown":
            verdict += f" 由动态策略选择（{md_escape(c.policy_label)}）。"
        if not c.sniffed:
            verdict += " 未嗅探到域名（可能为 IP 直连或无 SNI）。"
...
def render_html(title: str, markdown_text: str) -> str:
    """
    Self-contained HTML.
    Minimal Markdown rendering for readability (headings, lists, code blocks, inline code).
    """
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Generate Markdown/HTML report from daed logs.")
    ...
    if not wrote_any:
        print("Nothing written (both --no-md and --no-html set).", file=sys.stderr)
        return 2
